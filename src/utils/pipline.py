...
class piplineCommand(baseCommand):
    ...
    def handle(self, args):
        if args.lmdbPath is None:
            ...
            ExtractCommand.add_arguments(parser_lmdb)
            for file in os.listdir(args.genomesDir):
                if file.endswith('.fasta') or file.endswith('.faa'):
                    ...
                    run(args_lmdb)
            args.lmdbPath = args.outputPath.joinpath('lmdb_' + args.name)
...
class piplineModule:
    ...
    def process(self, ):
        ...
        # predict BGC
        args.datasetPath = self.outputPath.joinpath(self.name + '_split.csv')
